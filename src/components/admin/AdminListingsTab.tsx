...
import { AdminListingEditDialog, type AdminListingItem } from './AdminListingEditDialog';
...
export function AdminListingsTab() {
  ...
  const [editingItem, setEditingItem] = useState<AdminListingItem | null>(null);
  ...
  const handleDelete = async (item: AdminListingItem) => {
  ...
  const openEdit = (item: AdminListingItem) => {
    setEditingItem(item);
    setEditOpen(true);
  };
