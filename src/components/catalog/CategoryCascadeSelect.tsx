...
export function CategoryCascadeSelect({ tree, value, onChange, disabled }: CategoryCascadeSelectProps) {
  ...
  const selectedChildLabel =
    selectedChild?.name ?? (hasChildren ? 'Selecciona subcategoría (opcional)' : '— Sin subcategorías —');

  const handleParentSelect = (uuid: string) => {
    setParentUuid(uuid);
    setParentOpen(false);
    // Siempre asigna la categoría principal; la subcategoría es opcional y la sobreescribe si se elige
    onChange(uuid);
  };
  ...
  return (
    <div className="space-y-2">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          <Label className="text-slate-600 font-semibold">Subcategoría (opcional)</Label>
          <Popover open={childOpen} onOpenChange={setChildOpen}>
            <PopoverTrigger asChild>
              <Button
                type="button"
                variant="outline"
                disabled={disabled || !parentUuid || !hasChildren}
                className="h-12 w-full justify-between rounded-xl border-slate-200 bg-slate-50 font-normal"
              >
      ...
      <p className="text-xs text-slate-400">
        Selecciona una categoría principal para continuar. La subcategoría es opcional.
      </p>
    </div>
  );
}
