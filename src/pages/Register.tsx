...
export function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to home/dashboard assuming register auto-logs in
      navigate("/");
    } catch (err: any) {
      ...
      if (data?.errors && Array.isArray(data.errors)) {
        // Formatear los errores de Zod devueltos por tu backend
      ...
      setError(`Rechazado por el backend: ${message}`);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <form className="mt-10 space-y-6" onSubmit={handleRegister}>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-slate-800">Nombre(s)</label>
                    <Input
                      value={firstName}
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-slate-800">Doc. Identidad (Cédula)</label>
                    <Input
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="h-14 w-full rounded-2xl bg-[#e86f00] hover:bg-[#d46500] text-white font-bold disabled:opacity-70"
                >
